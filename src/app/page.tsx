...
export default function StreamlinedPortfolio() {
  return (
      ...
      <div className="max-w-6xl mx-auto py-12 px-6 lg:py-24 space-y-32">
        ...
        <header className="space-y-10">
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            <a href="mailto:harshrajput18@gmail.com" className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black hover:bg-emerald-600 transition-all flex items-center gap-3 uppercase text-sm shadow-xl shadow-slate-200 active:scale-95">
              <Mail size={18} /> Hire Me
            </a>
            <div className="flex items-center gap-8 px-8 border-l border-slate-200">
              <a href="https://github.com/harshrajput018" target="_blank" className="text-slate-400 hover:text-slate-900 transition-all hover:scale-110"><Github size={28} /></a>
              <a href="https://linkedin.com/in/harsh-rajput-0542a4185/" target="_blank" className="text-slate-400 hover:text-slate-900 transition-all hover:scale-110"><Linkedin size={28} /></a>
            </div>
          </div>
        </header>
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {projects.map((p, i) => (
              <a key={i} href={p.link} target="_blank" className={`group p-10 rounded-[3rem] border transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ${p.color}`}>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {p.tech.map(t => (
                      <span key={t} className="px-3 py-1 bg-white/80 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-500 uppercase shadow-sm">{t}</span>
                    ))}
                  </div>
                </div>
              </a>
              ...
              <div key={i} className="p-8 rounded-[2.5rem] bg-slate-50 border border-slate-200/60 hover:border-emerald-500/30 transition-all duration-500 group">
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.skills.map(skill => (
                    <span key={skill} className="px-3 py-1.5 bg-white border border-slate-100 rounded-lg text-xs font-bold text-slate-600 shadow-sm">{skill}</span>
                  ))}
                </div>
              </div>
            ))}
